...
function view(){
    /*
     * выделенные объекты, относительно которых строится всё пространство объектов. Они формируют точку
     * зрения. Это как ключевые слова для поиска
     */
    var selected = [];
    ...
    /*
     * Добавление объекта к списку объектов, которые являются точками отсчёта
     */
    this.add_selected = function(object){
	//добавляем к массиву выбранных
	/*
	 *производим глубокий поиск всех объектов, которые пересекаются с другими выбранными объектами,
	 * поиск производится со всех направлений - этого объекта и других выбранных объектов. Таким
	 * образом находятся те объекты, которые находятся между этим и всеми другими выбранными
	 * объектами.
	 * 
	 * Расставляем найденные объекты на холсте так, чтобы они находились ближе к тем объектам, к которым
	 * они ближе. Таким образом объекты, которые близки сразу к нескольким объектам расстанавливаются 
	 * между ними. А те, что близки к одном или двум, ставятся только близи них.  
	 */
    };
    /*
...
module.exports = function(dsa, stack){
	    ...
	    if(typeof view.old != 'undefined')
		return;
	    new view(stack);
	    
	    var object_to_find = new entry({
					       height : 1,
					       width : 2,
					       advertisement : 'искомый объект',
					       on_click : function(stack){
						   var founded_object = objects.find(object_to_find.get_value());
						   if(founded_object)
						       view.add_selected(founded_object);
					       }
					   });
